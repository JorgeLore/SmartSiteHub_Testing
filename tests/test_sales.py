...
class TestSales:
    ...
    @pytest.mark.parametrize("products", get_random_product_sets(num_tests=5, products_per_test=1))
    ...
    def test_remove_current_ticket(self, sales_page):
        """TC-SALES-100: Remove item and ensure total updates correctly."""
        sales_page.start_new_ticket()
        sales_page.start_new_ticket()
        sales_page.start_new_ticket()
    ...
    def test_cancel_remove_current_ticket(self, sales_page):
        ...
        sales_page.start_new_ticket()
        sales_page.start_new_ticket()
        sales_page.cancel_remove_current_ticket()
        tickets = sales_page.get_ticket_list()
    ...
    @pytest.mark.tc_sales_009
    def test_save_pending_ticket(self, sales_page):
        """TC-SALES-004: Save ticket as pending and verify it appears in list."""
        sales_page.start_new_ticket()
        sales_page.add_product_by_code("CODE123")
        sales_page.save_ticket_as_pending()
        pending_tickets = sales_page.get_pending_tickets()
        assert any("CODE123" in ticket for ticket in pending_tickets)

    @pytest.mark.tc_sales_022
    def test_checkout_ticket_and_generate_receipt(self, sales_page):
        """TC-SALES-005: Complete sale and verify receipt is generated."""
        sales_page.start_new_ticket()
        sales_page.add_product_by_code("CODE123")
        sales_page.checkout_current_ticket()
        assert sales_page.is_receipt_generated()